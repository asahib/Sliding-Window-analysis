function DynamicBC_run_check(subj_no)
...
%SLD_window=[10 20 30 40 50 60 80 100];
for subj=F
%     for subd=1
        for winsz=2  %% 20 samples ideal for TR 0.66 s and above
            
            spm8final 'pathonly';
            addpath '/nic/sw/spm/toolbox/rest';
            spmdir='/p/ashish/Multiband_prisma/HCP'; %% home directory of subjects 
           subjects={'100307','103414','110411','111312','113619','115320','117122','123117','124422','125525','129028','133928','135932','136833','139637'};

            SLD_window=[10 20 30 40 50 60 80 100]; %%% window size in samples
            %             E = F.E;
            R = [1;0];%cell2mat(get(E.rd_tvmodel,'val')); %sld,fls
            R1 = [0;0;1];% cell2mat(get(E.rd_rvw,'val')); %voxels ROI FCD
            R2 = [0;1;0;0;0];%cell2mat(get(E.rd_mr,'val')); %Default mask, User-Defined Mask,Nifti Label,TXT,Mat
            flag_seed_ROI = [0;1];%cell2mat(get(E.rd_seed_ROI,'val'));
            DataProcessDir=fullfile(spmdir,subjects{subj},'DY_bc_st');
            % DataProcessDir = strcat(get(E.ed_bg_data_dir,'string'));
            save_dir=fullfile(spmdir,subjects{subj},sprintf('OP_DY_bc_3mm_VARthr_%d',SLD_window(winsz)));
            % save_dir = get(E.ed_bg_dir_out,'string');
            save_info.save_dir = save_dir;
            prefix_name ='TV';% get(E.ed_prefix(1),'string');
            prefix_name_flag = 2;%length(strcat(prefix_name));
            ...
            if R(1) % sliding windows
                slw_id =1;% find(cell2mat(get(E.rd_slw,'val'))>0);
                slw_alignment = 'Ahead';%get(E.rd_slw(slw_id),'string');
                slw_winsize=SLD_window(winsz);
                %     slw_winsize = str2num(get(E.ed_rd11_tvmodel(1),'string'));
                slw_overlapsize = 0.2;%str2num(get(E.ed_rd11_tvmodel(2),'string'));
                slw_order =1; %str2num(get(E.ed_rd11_tvmodel(3),'string'));
                save_info.slw_alignment = slw_id;
                pvalue =1.0000e-03;% str2num(get(E.ed_slw_pvalue,'string'));
                fwe_flag =0;% get(E.rd_slw_conn(1),'val');
            ...
            if any([R1([1 3]); (R1(2)&R2(3))]) % seed FC/ FCD
                ...
                if R2(1) % default mask
                    ...
                    mask_label_template=(spm_select('FPList', fullfile(spmdir,subjects{subj}, 'T1'),'^gray_mask_0.2.*\.img'));
                end
